...
def run_epoch(model, loader, seg_crit, cls_crit, optimizer, scheduler, device,
              train: bool, seg_loss_weight: float, cls_loss_weight: float,
              use_cls_head: bool, scaler=None):
    ...
    with ctx:
        for batch in loader:
            ...
            amp_ctx = torch.autocast(device_type=device.type, enabled=scaler is not None)
            with amp_ctx:
                out = model(image)
...
def main():
    ...
    print(f"Computed pos_weights from TRAIN split: cls={cls_pos_weight:.2f} seg={seg_pos_weight:.2f}")
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        t0 = time.time()
        train_metrics = run_epoch(model, train_loader, seg_crit, cls_crit, optimizer,
        ...
        val_metrics = run_epoch(model, val_loader, seg_crit, cls_crit, optimizer,
        ...
        row = {"epoch": epoch, "time_s": round(dt, 1),
        ...
        if val_metrics["dice@0.5"] > best_val_dice:
            ...
            torch.save({"model_state": model.state_dict(), "epoch": epoch,
                        "val_dice": best_val_dice, "config": cfg},
                       out_dir / "best_model.pt")
        else:
            epochs_no_improve += 1

        if epochs_no_improve >= cfg["training"]["early_stopping_patience"]:
            print(f"Early stopping at epoch {epoch} (no val_dice improvement for "
                  f"{cfg['training']['early_stopping_patience']} epochs). "
                  f"Best val_dice={best_val_dice:.4f}")
            break

    pd.DataFrame(history).to_csv(out_dir / "training_history.csv", index=False)
    (out_dir / "final_summary.json").write_text(json.dumps({
        "best_val_dice": best_val_dice,
        "cls_pos_weight": cls_pos_weight,
        "seg_pos_weight": seg_pos_weight,
    }, indent=2))
    print(f"Done. Best val_dice={best_val_dice:.4f}. Checkpoint: {out_dir / 'best_model.pt'}")
