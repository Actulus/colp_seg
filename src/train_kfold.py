...
def build_fold_manifest(
    full_df: pd.DataFrame, test_cases: set, val_cases: set, out_path: Path
) -> None:
    df = full_df.copy()
...
def train_one_fold(
    cfg: dict, manifest_path: str, device: torch.device, fold_out_dir: Path
) -> dict:
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        if epochs_no_improve >= cfg["training"]["early_stopping_patience"]:
            print(
                f"    early stopping at epoch {epoch}, best_val_dice={best_val_dice:.4f}"
            )
            break

    pd.DataFrame(history).to_csv(fold_out_dir / "training_history.csv", index=False)
    torch.save(
        {"model_state": best_state, "val_dice": best_val_dice},
        fold_out_dir / "best_model.pt",
    )
    return {"model_state": best_state, "best_val_dice": best_val_dice}


def evaluate_fold(
    cfg: dict, manifest_path: str, model_state: dict, device: torch.device
) -> dict:
...
def run_kfold(cfg: dict, n_folds: int, seed: int = 42):
    ...
    cases = (
        full_df[full_df["source"] == "annocerv"][["case_id", "label"]]
        .drop_duplicates()
    ...
    for fold_i, (trainval_idx, test_idx) in enumerate(fold_indices):
        ...
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=0.15, random_state=seed + fold_i
        )
        train_i, val_i = next(
            sss.split(trainval_cases["case_id"], trainval_cases["label"])
        )
        ...
        print(
            f"  train={n_train} cases, val={len(val_cases)} cases, test={len(test_cases)} cases"
        )
        ...
        fold_results.append(row)
        print(
            f"  fold {fold_i} test_dice={row['test_dice_mean']:.4f} "
    ...
    print("\n=== K-fold summary ===")
    print(
        f"test_dice: {summary['test_dice_mean']:.3f} +/- {summary['test_dice_std']:.3f}"
    )
    print(
        f"dice_by_grade low:  {summary['dice_by_grade_low_mean']:.3f} +/- {summary['dice_by_grade_low_std']:.3f}"
    )
    print(
        f"dice_by_grade high: {summary['dice_by_grade_high_mean']:.3f} +/- {summary['dice_by_grade_high_std']:.3f}"
    )
